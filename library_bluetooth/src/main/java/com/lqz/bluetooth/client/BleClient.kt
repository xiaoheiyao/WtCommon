...
internal class BleClient(override val context: Context,
    ...
    private val gattCallback = object : BluetoothGattCallback() {
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            if(status == BluetoothGatt.GATT_SUCCESS){
                ...
                if(gattService != null){
                    ...
                    Log.e("Bluetooth","$logTag --> onServicesDiscovered: getService($serviceUUID)=null")
                    callConnectState(ConnectState.CONNECT_ERROR)
                }
            } else {
                Log.e("Bluetooth","$logTag --> onServicesDiscovered: status=$status")
                callConnectState(ConnectState.CONNECT_ERROR)
            }
        }
        ...
        override fun onCharacteristicWrite(gatt: BluetoothGatt,
            ...
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
                Log.d("Bluetooth","$logTag --> onCharacteristicWrite: value=${String(value)}, status=$status")
                callSendDataResult(value, status == BluetoothGatt.GATT_SUCCESS)
            } else {
                //api33及以上获取不到value
                Log.d("Bluetooth","$logTag --> onCharacteristicWrite: value=${value ?: null}, status=$status")
            }
        }
        ...
        override fun onCharacteristicRead(gatt: BluetoothGatt,
            ...
            @Suppress("DEPRECATION")
            val value = characteristic.value
            Log.d("Bluetooth","$logTag --> onCharacteristicRead: value=${value.contentToString()}, status=$status")
        ...
        override fun onCharacteristicRead(gatt: BluetoothGatt,
            ...
            Log.d("Bluetooth","$logTag --> onCharacteristicRead2: value=${value.contentToString()}, status=$status")
            callReadDataResult(characteristic.uuid, status == BluetoothGatt.GATT_SUCCESS, value)
        }

        override fun onMtuChanged(gatt: BluetoothGatt, mtu: Int, status: Int) {
            Log.d("Bluetooth","$logTag --> onMtuChanged: status=${status}, mtu=$mtu")
    ...
    private fun discoverServices(){
        if(!bluetoothGatt!!.discoverServices()){
            Log.e("Bluetooth","$logTag --> discoverServices=false")
            callConnectState(ConnectState.CONNECT_ERROR)
        }
    }
    ...
    override fun changeMtu(mtu: Int): Boolean {
        ...
        Log.d("Bluetooth","$logTag --> requestMtu($mtu)=$mtuResult")
        return mtuResult
    }
    ...
    private fun getGattService(): BluetoothGattService?{
        if(bluetoothGatt == null){
            Log.e("Bluetooth","$logTag --> 设备未连接!")
            return null
        }
        if(serviceUUID == null){
            Log.e("Bluetooth","$logTag --> 未设置serviceUUID!")
            return null
        }
        val gattService = bluetoothGatt!!.getService(serviceUUID)
        if(gattService == null){
            Log.e("Bluetooth","$logTag --> getService($serviceUUID)=null!")
            return null
        }
        return gattService
    }
    ...
    override fun receiveData(uuid: UUID?, onReceive: (ByteArray) -> Unit): Boolean{
        ...
        if(readCharacteristic == null){
            Log.e("Bluetooth","$logTag --> receiveData: getCharacteristic($uuid)=null")
            return  false
        }
        val notificationResult = bluetoothGatt!!.setCharacteristicNotification(readCharacteristic, true)
        Log.d("Bluetooth","$logTag --> receiveData: notificationResult=$notificationResult")
    ...
    override fun sendData(uuid: UUID?, data: ByteArray, timeoutMillis: Long, callback: DataResultCallback) {
        ...
        scheduleTimeoutTask(timeoutMillis){
            Log.e("Bluetooth","$logTag --> sendData timeout")
        ...
        if(writeCharacteristic != null){
            if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) {
                ...
                if(!bluetoothGatt!!.writeCharacteristic(writeCharacteristic)){
                    Log.e("Bluetooth","$logTag --> sendData: writeCharacteristic=false")
                    callSendDataResult(data, false)
                }
            } else {
                val result = bluetoothGatt!!.writeCharacteristic(writeCharacteristic, data, writeType)
                Log.e("Bluetooth","$logTag --> sendData: writeCharacteristic=$result")
            ...
            Log.e("Bluetooth","$logTag --> sendData: getCharacteristic($uuid)=null")
            callSendDataResult(data, false)
        }
    }
    ...
    override fun readData(uuid: UUID?, timeoutMillis: Long, callback: DataResultCallback) {
        ...
        scheduleTimeoutTask(timeoutMillis){
            Log.e("Bluetooth","$logTag --> readData timeout")
            callReadDataResult(uuid, false)
        }
        val readCharacteristic = gattService.getCharacteristic(uuid)
        if(readCharacteristic != null){
            if(!bluetoothGatt!!.readCharacteristic(readCharacteristic)){
                Log.e("Bluetooth","$logTag --> readData: readCharacteristic=false")
                callReadDataResult(uuid, false)
            }
        } else {
            Log.e("Bluetooth","$logTag --> readData: getCharacteristic($uuid)=null")
            callReadDataResult(uuid, false)
        }
    }
