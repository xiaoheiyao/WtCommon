...
internal class ClassicClient(override val context: Context,
    ...
    private val stateReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            Log.d("Bluetooth","$logTag --> stateReceiver: ${intent.action}")
            when (intent.action) {
...
//                    Log.d("Bluetooth","$logTag --> stateReceiver, bondState: $bondState")
//                    when (bondState) {
...
//                }
    ...
    override fun startScan(callback: ScanDeviceCallback) {
        ...
        bluetoothAdapter.bondedDevices.forEach {
            Log.d("Bluetooth","$logTag --> 已配对设备：$it, uuids=${it.uuids?.contentToString()}")
...
//            receiverFilter!!.addAction(BluetoothDevice.ACTION_BOND_STATE_CHANGED)
    ...
    private fun createRfcommSocketToConnect(): Boolean{
        return try {
            ...
            Log.e("Bluetooth","$logTag --> createRfcommSocketToConnect failed")
            false
        }
    }
    ...
    private fun checkClientValid(): Boolean{
        ...
        if(bluetoothSocket == null){
            Log.e("Bluetooth","$logTag -->bluetoothSocket Not connected")
            return false
        }
        return true
    }
        ...
        thread {
            ...
            while (bluetoothSocket?.isConnected == true && readFailedCount < 3){
                try {
                    ...
                    Log.e("Bluetooth","$logTag --> receiveData failed ${++readFailedCount}")
                }
            }
    ...
    override fun sendData(uuid: UUID?, data: ByteArray, timeoutMillis: Long, callback: DataResultCallback) {
        ...
        try {
            ...
            Log.e("Bluetooth","$logTag --> sendData failed")
            cancelTimeoutTask()
            callback.call(false, data)
        }
    }
    ...
    override fun readData(uuid: UUID?, timeoutMillis: Long, callback: DataResultCallback) {
        ...
        thread {
            try {
                ...
                Log.e("Bluetooth","$logTag --> readData failed")
                cancelTimeoutTask()
                callback.call(false, null)
            }
        }
