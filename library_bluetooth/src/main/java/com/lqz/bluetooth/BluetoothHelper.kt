...
internal object BluetoothHelper {
            ...
            if(intent.action == BluetoothAdapter.ACTION_STATE_CHANGED){
                when(intent.getIntExtra(BluetoothAdapter.EXTRA_STATE, 0)){
                    BluetoothAdapter.STATE_TURNING_ON -> {
                        Log.d("Bluetooth","$logTag --> 蓝牙正在打开...")
                    }
                    BluetoothAdapter.STATE_ON -> {
                        Log.d("Bluetooth","$logTag --> 蓝牙已经打开。")
                        turnOn()
                    }
                    BluetoothAdapter.STATE_TURNING_OFF -> {
                        Log.d("Bluetooth","$logTag --> 蓝牙正在关闭...")
                        turnOff()
                    }
                    BluetoothAdapter.STATE_OFF -> {
                        Log.d("Bluetooth","$logTag --> 蓝牙已经关闭。")
                    }
                }
            }
        }
    ...
    fun switchBluetooth(context: Context, bluetoothAdapter: BluetoothAdapter, enable: Boolean,
        ...
        if(Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU){
            if(enable){
                Log.d("Bluetooth","$logTag --> 请求开启蓝牙")
                @Suppress("DEPRECATION")
                return bluetoothAdapter.enable()
            } else {
                Log.d("Bluetooth","$logTag --> 请求关闭蓝牙")
                @Suppress("DEPRECATION")
    ...
    private fun checkPermissions(context: Context, request: Boolean = true): Boolean {
        for (permission in permissions) {
            ...
            if (grant != PackageManager.PERMISSION_GRANTED) {
                Log.d("Bluetooth","$logTag --> 缺少权限$permission, 尝试申请...")
